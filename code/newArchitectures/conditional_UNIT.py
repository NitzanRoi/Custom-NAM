...
def weights_init(m):
    ...
    cls_problems = ['AdaptiveInstanceNorm2d','ResBlock','LayerNorm','Conv2dBlock'
                    ,'ResBlocks','ContentEncoder','Decoder','VAEGen']
    if classname not in cls_problems:
...
class _netT64(nn.Module):
    def __init__(self, ngf, in_nc, out_nc):
        ...
        # taken from 'unit_edges2shoes_folder.yaml':
        hyperparameters = {
            'input_dim_a':3,
            'gen': {
                'dim':64,
        ...
        self.gen_a = VAEGen(hyperparameters['input_dim_a'], hyperparameters['gen'])  # auto-encoder

    def forward(self, Is):
        return self.gen_a.dec(self.gen_a.enc(Is))
